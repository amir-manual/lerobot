...
class ACTConfig(PreTrainedConfig):
    ...
    # Auxiliary heads: optional task-phase classification + finger-state regression + subgoal-end
    # (in-phase progress) regression, trained alongside the primary action loss so a later eval can
    # compare predicted-vs-actual phase/finger-state/progress to distinguish planning failures
    # (wrong macro-phase) from execution failures (right phase, wrong finger state), SADP/SeqVLA-
    # style. `False` is a no-op -- no extra heads are built, no extra batch keys are read, and
    # forward()'s return signature is unchanged.
