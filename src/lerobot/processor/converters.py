...
_COMPLEMENTARY_KEYS = (
    ...
    # Auxiliary supervision targets for ACT's/GR00T's optional phase/finger-state/subgoal-end
    # heads (see ACTConfig/GrootConfig.predict_aux_heads). Neither an "observation.*" feature nor
    # the primary "action" key, but must still survive batch_to_transition/transition_to_batch's
    # round-trip the same way "task"/"episode_index" already do -- this allowlist is the mechanism
