...
**`prefix_guidance`**: Whether to condition each chunk on the previous chunk's unexecuted tail (default: `True`). Setting it to `False` keeps asynchronous chunk production — and therefore latency hiding — but drops the overlap conditioning. Use it as an ablation to measure how much the prefix actually smooths chunk boundaries.
