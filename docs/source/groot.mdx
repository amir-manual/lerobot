...
Every chunk swap is measured, so you can compare configurations rather than eyeball smoothness.
At the end of each episode the rollout logs the commanded-action jump across chunk boundaries in
raw action units:
...
Lower is smoother. To measure the effect of prefix guidance itself, run the same task twice with
the same `execution_horizon` and fps and compare `max_abs mean`:

```bash
# with seam conditioning (default)
--inference.type=rtc --inference.rtc.enabled=True --inference.rtc.prefix_guidance=true
# same asynchronous chunk replacement, no seam conditioning
--inference.type=rtc --inference.rtc.enabled=True --inference.rtc.prefix_guidance=false
```

`prefix_guidance=false` only removes the overlap conditioning; chunks are still produced ahead of
time, so latency hiding — and therefore the arm's motion continuity between chunks — is otherwise
